...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockRepository.discoverMedia(
      ...
      MediaItem(id: 11, title: 'Grid Movie 1', overview: '', releaseDate: '2024', mediaType: MediaType.movie),
      MediaItem(id: 12, title: 'Grid Movie 2', overview: '', releaseDate: '2024', mediaType: MediaType.movie),
      MediaItem(id: 13, title: 'Grid Movie 3', overview: '', releaseDate: '2024', mediaType: MediaType.movie),
    ]);
  });

  testWidgets('Grid size slider updates grid count and seen overlay appears', (tester) async {
    ...
    await tester.pumpWidget(
      MultiProvider(
        ...
        child: MaterialApp(theme: DefaultLightPalette().toThemeData(), home: const DiscoveryPage()),
      ),
    );
    ...
    final delegate2 = grid2.gridDelegate as SliverGridDelegateWithFixedCrossAxisCount;
    expect(delegate2.crossAxisCount, equals(5));

    // Now simulate an item being marked seen by returning a seen item
    final seen = SeenItem(id: 1, tmdbId: 12, type: MediaType.movie, title: 'Grid Movie 2', seenDate: DateTime.now());
