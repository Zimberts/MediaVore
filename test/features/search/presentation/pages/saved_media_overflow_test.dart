...
void main() {
  ...
  testWidgets(
    'SavedMediaPage AppBar overflows with very long list name at small width',
    (WidgetTester tester) async {
      ...
      final hasOverflow = errors.any(
        (d) =>
            d.exceptionAsString().contains('overflowed') ||
            d.exceptionAsString().contains('Overflow'),
      );
      expect(
        hasOverflow,
        isFalse,
        reason:
            'Expected no RenderFlex overflow when selecting a very long list name',
      );
    },
  );
}
