...
void main() {
  ...
  group('SearchOverlay Requirements', () {
    testWidgets('search result has bookmark icon button', (
      WidgetTester tester,
    ) async {
      ...
      // Use deterministic key for the search overlay add button.
      final addButton = find.byKey(const ValueKey('watchlist-1-movie-search'));
    ...
    testWidgets('clear button resets search results', (
      WidgetTester tester,
    ) async {
      when(
        () => mockRepository.searchMedia(any(), page: any(named: 'page')),
