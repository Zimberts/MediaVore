          ...
          return AlertDialog(
            ...
            content: SizedBox(
              ...
              child: SingleChildScrollView(
                            ...
                            // Left-top type + like handled above
                            ...
                            Positioned(
                              ...
                              child: GestureDetector(
                                key: ValueKey('watchlist-${item.id}-${item.mediaType.name}-grid'),
                                onTap: () async {
                                ...
                                child: Container(
                                  ...
                                  child: Icon(provider.isItemInList(item, 'watchlist') ? Icons.bookmark : Icons.bookmark_border, color: colors.badgeText, size: 16),
                                ),
                              ),
                            ),
                            ...
                            if (isSeen)
                              Positioned(
                                ...
                                bottom: -4,
                                child: Container(
                                  decoration: BoxDecoration(color: isFinished ? colors.badgeBgSeen : colors.badgeBg, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 1.5)),
                                  padding: const EdgeInsets.all(2),
                                  child: Icon(isFinished ? Icons.done_all : Icons.check, size: 10, color: colors.badgeText),
                                ),
                              ),
                            return Padding(
                          ...
                          children: [
                            if (item.voteAverage != null && item.voteAverage! > 0) ...[
                              Container(
                                ...
                                child: Row(
                                  children: [
                                    Icon(Icons.star, color: colors.ratingStar, size: 10),
                                    const SizedBox(width: 2),
                                    Text(item.voteAverage!.toStringAsFixed(1), style: const TextStyle(color: Colors.white, fontSize: 10)),
                                  ],
                                ),
                              ),
                            ...
                            if (_canNotify(item)) ...[
                              GestureDetector(
                                key: ValueKey('notify-${item.id}-${item.mediaType.name}-grid'),
                                onTap: () async {
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(6)),
                                  child: Icon(
                                    provider.isNotified(item) ? Icons.notifications_active : Icons.notifications_none,
                                    color: provider.isNotified(item) ? colors.info : colors.badgeText,
                                    size: 14,
                                  ),
                                ),
                              ),
                          ...
                          child: Row(
                            ...
                            children: [
                              Container(
                                padding: const EdgeInsets.all(2),
                                decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(4)),
                                child: Icon(item.mediaType == MediaType.tv ? Icons.tv : Icons.movie, color: Colors.white, size: 10),
                              ),
                              const SizedBox(width: 6),
                              GestureDetector(
                                key: ValueKey('like-${item.id}-${item.mediaType.name}-grid'),
                                onTap: () async {
                                  await provider.toggleLike(item);
                                  setState(() {});
                                },
                                child: Icon(
                                  provider.isLiked(item) ? Icons.favorite : Icons.favorite_border,
                                  color: provider.isLiked(item) ? colors.likeHeart : colors.badgeText,
                                  size: 12,
                                ),
                              ),
                            ],
